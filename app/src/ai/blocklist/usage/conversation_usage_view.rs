...
/// This backs the terminal's usage footer.
...
impl ConversationUsageView {
    /// Constructs the footer view without an orchestration credit rollup.
    ///
    /// Production always goes through [`Self::new_footer_with_rollup`]; this
    /// exists so tests can build the view without a window or a parent
    /// conversation.
    #[cfg(test)]
    pub fn new(
    ...
    /// Constructs the footer view with orchestration credit rollup wired in.
    ...
    /// locally-loaded descendants with
    /// non-zero credits. The feature is self-gating: conversations without
    /// descendants short-circuit before any rollup-specific UI is built, so no
