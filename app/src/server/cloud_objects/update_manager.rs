...
impl UpdateManager {
    ...
    pub fn received_message_from_server(
        ...
        match message {
            ...
            ObjectUpdateMessage::AmbientTaskUpdated { .. } => {
                // RTC push for ambient tasks (`FeatureFlag::AmbientAgentsRTC`) is folded
                // off; the remaining consumers re-fetch on their own schedules.
            }
        }
    }
    ...
    #[allow(clippy::too_many_arguments)]
    // Its production callers went with the scheduled-ambient-agent CLI; the
    // online object-create path goes with the cloud_objects layer.
    #[allow(dead_code)]
