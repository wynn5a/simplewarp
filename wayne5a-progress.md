...
- Recommended round order (4ca): (1) ✅orphaned public_api deletion (4cb); (2) ✅artifacts group (4cc); (3) ✅warp api-key round (4cd); (4) ✅StoreClient + full_source_code_embedding (4ce); (5) ✅assistant surfaces 4 (4cf); (6) ✅conversation-sync 10 walls (4cg) / ✅cloud-run lifecycle (4ch); (7) telemetry scope decision; (8) the fold + crates.
- 4ch done (2026-09-18): cloud-run lifecycle COMPLETE — the 11 remaining AIClient walls and `server_api/ai.rs` whole (so `AIClient` is gone as a trait), the ambient terminal UI vertical, the Claude/Codex transcript upload modules, and the whole HTTP retry layer (`server/retry_strategies.rs` + `ai/agent_sdk/retry.rs`), plus the dead-code cascade in both tiers (`cargo check` warnings and the `clippy --all-targets` non-test tier whose only callers were their own unit tests). warp lib nextest 4,658 default / 4,657 simplewarp, 0 fail. Compiler-invisible cloud-run residue left for the next slice: `/cloud-agent` still active locally, `/host` + `/harness` permanently inactive, `QueuedQueryOrigin::InitialCloudMode` queue branches, ~249 ambient task-id plumbing refs. Ledger: plan.md 4ch entry.
