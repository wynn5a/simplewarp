...
## Next candidates
- 4by done (2026-09-16): login-gated refresh group COMPLETE — get_request_limit_info + get_available_harnesses chains (45 files, +190/−1,843; AIRequestUsageModel reduced to defaults + refund path, BonusGrant/quota-banner/BonusGrantNotificationModel/ambient-banner chains gone, HarnessAvailabilityModel static-Oz, FeaturePopup::AlertIcon deleted). AIClient 31 → 29. Ledger: plan.md 4by entry. 4bx recorded too (was missing from the ledger).
- Next: AuthClient (12 methods — 4bn took the first wave, survey in ledger), then ServerApi/Provider/BaseClient; the provide_negative_feedback refund path is the one remaining caller of AIRequestUsageModel's ai_client (takes the model to a client-less singleton). After the client chain: feature rounds for local-but-disabled flags (enable-candidates: EditableMarkdownMermaid, ImeMarkedText, ITermImages).
